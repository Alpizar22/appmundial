...
export function renderAutomationSubworld({ctx,time,viewport,focus,intensity,palette}) {
  ...
  for(let row=0;row<rows;row++){
    ...
    for(let column=0;column<columns;column++){
      const across=column/(columns-1),nx=across*2-1,localDepth=familyDepth+rowWarp+valueNoise(nx*(1.05+family*.19)+family*2.7,familyDepth*2.4)*.14,verticalBreath=.82+Math.sin(time*(.24+family*.018)+family*1.31)*.28,heightField=duneHeight(nx*(.76+hash2(family,8.4)*.64),localDepth,time*rowSpeed)*rowAmplitude*verticalBreath*(1+within*.055),ridgeLift=heightField*lerp(height*.1,height*.42,perspective),localBurst=.55+valueNoise(nx*2.2+family,familyDepth*3.4+time*.12)*.9,familyFlow=Math.sin(time*.68*rowSpeed+familyDepth*3.1+nx*(1.15+family*.31))+Math.sin(time*.39-familyDepth*2.4+nx*(2.1+family%3*.65))*.36,sandDrift=familyFlow*lerp(5,18,perspective)*localBurst+within*1.4
