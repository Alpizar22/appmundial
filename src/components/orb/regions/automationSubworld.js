...
export function renderAutomationSubworld({ctx,time,viewport,focus,intensity,palette}) {
  ...
  for(let row=0;row<rows;row++){
    ...
    for(let column=0;column<columns;column++){
      const across=column/(columns-1),nx=across*2-1,lateralPhase=time*(1.72+rowSpeed*.5)-nx*(4.1+family*.3),lateralTravel=Math.sin(lateralPhase+family*.91)+Math.sin(lateralPhase*.43-family*.63)*.42+Math.sin(lateralPhase*1.83+family*.47)*.22,localDepth=familyDepth+rowWarp+valueNoise(nx*(1.05+family*.19)+family*2.7,familyDepth*2.4)*.14,verticalBreath=.94+Math.sin(time*(.19+family*.012)+family*1.31)*.16,elevationWave=Math.sin(time*.31+family*.88-nx*.72)*(.07+hash2(family,71.9)*.08),heightField=(duneHeight(nx*(.76+hash2(family,8.4)*.64),localDepth,time*rowSpeed)+elevationWave)*rowAmplitude*verticalBreath*(1+within*.055),ridgeLift=heightField*lerp(height*.1,height*.42,perspective),localBurst=.55+valueNoise(nx*2.2+family,familyDepth*3.4+time*.12)*.9,sandDrift=lateralTravel*lerp(34,92,perspective)*(width/1440)*localBurst+within*1.4
  ...
  grid.forEach((points,row)=>{const depth=row/(rows-1),averageHeight=points.reduce((sum,point)=>sum+Math.max(0,point.height),0)/points.length,elevation=Math.min(1,averageHeight/.48),layerVisibility=lerp(.62,1.08,depth);ctx.strokeStyle=rgba(depth>.7?pearl:smoke,lerp(.12,.68,depth)*lerp(.58,1,elevation)*intensity);ctx.lineWidth=lerp(.42,1.28,depth)+elevation*.42;ctx.shadowColor=rgba(pearl,elevation*.18*layerVisibility*intensity);ctx.shadowBlur=elevation*4;let segment=[];points.forEach((point,index)=>{const connected=point.height>.035||index%7!==0;if(connected)segment.push(point);if((!connected||index===points.length-1)&&segment.length){strokeSpline(ctx,segment);segment=[]}});ctx.shadowBlur=0})
  ...
  for(let pulse=0;pulse<24;pulse++){const row=2+(pulse*7)%(rows-3),direction=pulse%3===0?-1:1,phase=frac(time*(.072+(pulse%5)*.006)+pulse*.137),travel=direction>0?phase:1-phase,position=travel*(columns-1),index=Math.min(columns-2,Math.floor(position)),local=position-index,a=grid[row][index],b=grid[row][index+1],envelope=Math.sin(phase*Math.PI)**.72,x=lerp(a.x,b.x,local),y=lerp(a.y,b.y,local),warm=pulse%4===0;ctx.fillStyle=rgba(warm?gold:pearl,envelope*lerp(.28,.82,a.depth)*intensity);ctx.beginPath();ctx.arc(x,y,lerp(.75,1.8,a.depth),0,Math.PI*2);ctx.fill()}
  grid.forEach((row,rowIndex)=>row.forEach((point,column)=>{if(rowIndex%2||column%2)return;const active=(rowIndex*13+column*7)%29===0,layerVisibility=lerp(.58,1.08,point.depth);ctx.fillStyle=rgba(active?gold:pearl,(active?.78:.34)*point.visibility*layerVisibility*intensity);ctx.beginPath();ctx.arc(point.x,point.y,active?1.65:lerp(.55,1.12,point.depth),0,Math.PI*2);ctx.fill()}))
  ctx.restore()
}
