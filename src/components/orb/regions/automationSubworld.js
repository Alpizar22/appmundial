...
export function renderAutomationSubworld({ctx,time,viewport,focus,intensity,palette}) {
  ...
  for(let row=0;row<rows;row++){
    ...
    for(let column=0;column<columns;column++){
      const across=column/(columns-1),nx=across*2-1,lateralPhase=time*(.82+rowSpeed*.24)-nx*(2.3+family*.17),lateralTravel=Math.sin(lateralPhase+family*.91)+Math.sin(lateralPhase*.47-family*.63)*.32,localDepth=familyDepth+rowWarp+valueNoise(nx*(1.05+family*.19)+family*2.7,familyDepth*2.4)*.14,verticalBreath=.94+Math.sin(time*(.19+family*.012)+family*1.31)*.16,elevationWave=Math.sin(time*.31+family*.88-nx*.72)*(.07+hash2(family,71.9)*.08),heightField=(duneHeight(nx*(.76+hash2(family,8.4)*.64),localDepth,time*rowSpeed)+elevationWave)*rowAmplitude*verticalBreath*(1+within*.055),ridgeLift=heightField*lerp(height*.1,height*.42,perspective),localBurst=.55+valueNoise(nx*2.2+family,familyDepth*3.4+time*.12)*.9,sandDrift=lateralTravel*lerp(11,34,perspective)*localBurst+within*1.4
