...
export function renderNasusOrb(ctx,model,{width,height,time,progress,mode,reduced,pointer}) {
  ...
  nodes.sort((a,b)=>a.z-b.z).forEach(node=>{const depth=(node.z+1)/2,important=node.principal||node.hub,variation=.78+node.seed*.44,r=(important?1.25+depth*.72:Math.max(.25,.34+depth*.94))*variation*(width<700?.86:1),opacity=important?.42+depth*.56:.1+depth*.72;ctx.fillStyle=rgba(important&&node.index%6===region?GOLD:PEARL,opacity*node.alpha);ctx.beginPath();ctx.arc(node.x,node.y,r+(activity*(important?.7:.12)),0,Math.PI*2);ctx.fill()})
}
