...
function paintWebWorld(ctx,nodes,model,weight,time) {
  ...
  districts.forEach(district=>{const originalCenter={...district.center},horizon=viewportHeight*.54,project=node=>{const rawDepth=clamp01((node.z+1)/2),near=.14+rawDepth*.86,spread=1.28+near*2.85,floorY=lerp(horizon,viewportHeight*1.01,near**1.28);return{...node,x:viewportWidth*.56+(node.x-originalCenter.x)*spread,y:floorY+(node.y-originalCenter.y)*(.16+near*.12),z:lerp(-.2,1,near),importance:Math.min(1,node.importance*.9+.16)}};district.nodes=district.nodes.map(project);district.center=project(originalCenter)})
