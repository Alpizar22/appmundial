...
function stepSimulation(model,time,profile,region,activity,reduced) {
  ...
  model.forEach((node,i)=>{node.age+=dt;if(node.state==='alive'&&node.age>node.life)node.state='dying';if(node.state==='dying'){node.alpha=Math.max(0,node.alpha-dt/.95);if(node.alpha===0){node.generation++;const u=hash(i+node.generation*17,21.3),v=hash(i+node.generation*23,31.7),theta=u*Math.PI*2,yy=v*1.7-.85,rr=Math.sqrt(1-yy*yy);node.x=Math.cos(theta)*rr;node.y=yy;node.z=Math.sin(theta)*rr;node.vx=(hash(i,node.generation+41)-.5)*.055;node.vy=(hash(i,node.generation+53)-.5)*.055;node.vz=(hash(i,node.generation+67)-.5)*.055;node.age=0;node.life=11+hash(i,node.generation+79)*12;node.state='born'}}else if(node.state==='born'){node.alpha=Math.min(1,node.alpha+dt/1.25);if(node.alpha===1)node.state='alive'}let length=Math.hypot(node.x,node.y,node.z)||1,nx=node.x/length,ny=node.y/length,nz=node.z/length;const drive=(profile.wander*.42)*(1+activity*.65);ax[i]+=(noise(i*.31+9,time*.17)-.5)*drive;ay[i]+=(noise(i*.53+27,time*.145)-.5)*drive;az[i]+=(noise(i*.77+55,time*.19)-.5)*drive;if(region===1)ax[i]+=.012*Math.sin(node.phase);if(region===3){ax[i]+=(Math.round(node.x*4)/4-node.x)*.024;ay[i]+=(Math.round(node.y*4)/4-node.y)*.024}if(region===4){const angle=(i%6)/6*Math.PI*2;ax[i]+=(Math.cos(angle)*.82-node.x)*.018;az[i]+=(Math.sin(angle)*.82-node.z)*.018}const forceNormal=ax[i]*nx+ay[i]*ny+az[i]*nz;ax[i]-=forceNormal*nx;ay[i]-=forceNormal*ny;az[i]-=forceNormal*nz;const velocityNormal=node.vx*nx+node.vy*ny+node.vz*nz;node.vx-=velocityNormal*nx;node.vy-=velocityNormal*ny;node.vz-=velocityNormal*nz;const damping=Math.exp(-(1.55-activity*.18)*dt);node.vx=(node.vx+ax[i]*dt)*damping;node.vy=(node.vy+ay[i]*dt)*damping;node.vz=(node.vz+az[i]*dt)*damping;const speed=Math.hypot(node.vx,node.vy,node.vz),maxSpeed=.085+activity*.025;if(speed>maxSpeed){const limit=maxSpeed/speed;node.vx*=limit;node.vy*=limit;node.vz*=limit}node.x+=node.vx*dt;node.y+=node.vy*dt;node.z+=node.vz*dt;length=Math.hypot(node.x,node.y,node.z)||1;node.x/=length;node.y/=length;node.z/=length;nx=node.x;ny=node.y;nz=node.z;const transportedNormal=node.vx*nx+node.vy*ny+node.vz*nz;node.vx-=transportedNormal*nx;node.vy-=transportedNormal*ny;node.vz-=transportedNormal*nz})
}
