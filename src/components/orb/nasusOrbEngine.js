...
function paintWebTower(ctx,node,weight,time,buildState) {
  const depth=Math.max(.03,(node.z+1)/2),fog=.055+smoothstep(depth)*.945,importance=.35+node.importance*.9,growth=smoothstep(weight),height=(20+node.seed*42+node.importance*82)*(.4+depth*.82)*growth,width=(5+node.importance*11)*(.5+depth*.76),lean=(node.seed-.5)*width*.7,topX=node.x+lean,topY=node.y-height,color=node.importance>.62?GOLD:PEARL,alpha=(.16+depth**1.55*.7)*weight*node.alpha*fog,archetype=Math.floor(node.seed*3),levels=3+Math.floor(height/19)+Math.floor(node.importance*2),rawBuild=Math.max(0,Math.min(1,(time-buildState.startedAt)/buildState.duration)),levelProgress=rawBuild*levels,builtLevels=Math.floor(levelProgress),segmentEase=smoothstep(frac(levelProgress)),reveal=Math.min(1,(builtLevels+segmentEase)/levels),revealY=lerp(node.y,topY,reveal)
...
function paintWebWorld(ctx,nodes,model,weight,time) {
  ...
  districts.forEach((district,districtIndex)=>{const spreadX=Math.max(48,Math.min(155,Math.max(...district.nodes.map(node=>Math.abs(node.x-district.center.x)))+24)),spreadY=Math.max(24,Math.min(82,Math.max(...district.nodes.map(node=>Math.abs(node.y-district.center.y)))+15)),fog=.08+smoothstep(Math.max(0,(district.center.z+1)/2))*.92;ctx.save();ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.clip();ctx.strokeStyle=rgba(SMOKE,.13*weight*fog);ctx.lineWidth=.42;for(let offset=-spreadX;offset<=spreadX;offset+=18){ctx.beginPath();ctx.moveTo(district.center.x+offset-spreadY*.35,district.center.y-spreadY);ctx.lineTo(district.center.x+offset+spreadY*.35,district.center.y+spreadY);ctx.stroke()}for(let offset=-spreadY;offset<=spreadY;offset+=14){ctx.beginPath();ctx.moveTo(district.center.x-spreadX,district.center.y+offset);ctx.lineTo(district.center.x+spreadX,district.center.y+offset*.72);ctx.stroke()}ctx.restore();ctx.strokeStyle=rgba(PEARL,.2*weight*fog);ctx.lineWidth=.55;ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.stroke();ctx.fillStyle=rgba(districtIndex===1?GOLD:PEARL,.5*weight*fog);ctx.beginPath();ctx.arc(district.center.x,district.center.y,1.5,0,Math.PI*2);ctx.fill()})

  const streetEdges=[];districts.forEach((district,districtIndex)=>{const streetKeys=new Set();district.nodes.forEach(node=>district.nodes.filter(peer=>peer.index!==node.index).sort((a,b)=>Math.hypot(a.x-node.x,a.y-node.y)-Math.hypot(b.x-node.x,b.y-node.y)).slice(0,2).forEach(peer=>{const key=node.index<peer.index?`${node.index}:${peer.index}`:`${peer.index}:${node.index}`;if(streetKeys.has(key))return;streetKeys.add(key);const depth=Math.max(.03,((node.z+peer.z)*.5+1)/2),fog=.06+smoothstep(depth)*.94;ctx.strokeStyle=rgba(PEARL,.24*fog*weight);ctx.lineWidth=.76;ctx.beginPath();ctx.moveTo(node.x,node.y);ctx.lineTo(peer.x,peer.y);ctx.stroke();streetEdges.push({a:node,b:peer,districtIndex,depth:fog})}))})
  ...
  for(let packet=0;packet<18&&streetEdges.length;packet++){const edge=streetEdges[(packet*7)%streetEdges.length],phase=frac(time*(.07+(packet%4)*.009)+packet*.119),envelope=Math.sin(phase*Math.PI),x=lerp(edge.a.x,edge.b.x,phase),y=lerp(edge.a.y,edge.b.y,phase);ctx.fillStyle=rgba(packet%5===0?GOLD:PEARL,envelope*edge.depth*.78*weight);ctx.beginPath();ctx.arc(x,y,.75+edge.depth*.68,0,Math.PI*2);ctx.fill()}
