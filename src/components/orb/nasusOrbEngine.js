...
function paintWebWorld(ctx,nodes,weight,time) {
  const dpr=ctx.getTransform().a||1,viewportWidth=ctx.canvas.width/dpr,viewportHeight=ctx.canvas.height/dpr
  const eligible=nodes.filter(node=>node.z>-.58&&node.alpha>.2&&node.x>viewportWidth*.12&&node.x<viewportWidth*.97&&node.y>viewportHeight*.015&&node.y<viewportHeight*.98),bounds=eligible.reduce((box,node)=>({minX:Math.min(box.minX,node.x),maxX:Math.max(box.maxX,node.x),minY:Math.min(box.minY,node.y),maxY:Math.max(box.maxY,node.y)}),{minX:Infinity,maxX:-Infinity,minY:Infinity,maxY:-Infinity}),columns=5,rows=4,buckets=Array.from({length:columns*rows},()=>[])
  eligible.forEach(node=>{const column=Math.min(columns-1,Math.floor((node.x-bounds.minX)/Math.max(1,bounds.maxX-bounds.minX)*columns)),row=Math.min(rows-1,Math.floor((node.y-bounds.minY)/Math.max(1,bounds.maxY-bounds.minY)*rows));buckets[row*columns+column].push(node)})
  buckets.forEach(bucket=>bucket.sort((a,b)=>(b.importance+b.seed*.22)-(a.importance+a.seed*.22)))
  const candidates=[],target=Math.min(42,eligible.length);for(let layer=0;candidates.length<target;layer++){let added=false;buckets.forEach(bucket=>{if(candidates.length>=target||!bucket[layer])return;candidates.push(bucket[layer]);added=true});if(!added)break}candidates.sort((a,b)=>a.z-b.z)
  const towers=candidates.map(node=>paintWebTower(ctx,node,weight,time))
  ...
  towers.forEach((tower,index)=>{const nearest=towers.filter(other=>other.index!==tower.index).sort((a,b)=>Math.hypot(a.x-tower.x,a.y-tower.y)-Math.hypot(b.x-tower.x,b.y-tower.y))[0],footprint=tower.width*1.35;ctx.strokeStyle=rgba(index%7===0?GOLD:SMOKE,(.12+tower.depth*.14)*weight);ctx.lineWidth=.48;ctx.beginPath();ctx.moveTo(tower.x-footprint,tower.y);ctx.lineTo(tower.x,tower.y+footprint*.35);ctx.lineTo(tower.x+footprint,tower.y);ctx.lineTo(tower.x,tower.y-footprint*.35);ctx.closePath();ctx.stroke();if(nearest&&tower.index<nearest.index){ctx.strokeStyle=rgba(PEARL,.105*weight*Math.max(.14,(tower.depth+nearest.depth)/2));ctx.beginPath();ctx.moveTo(tower.x,tower.y);ctx.lineTo(nearest.x,nearest.y);ctx.stroke()}})
