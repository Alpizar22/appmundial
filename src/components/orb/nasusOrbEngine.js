...
export function createOrbModel(count) {
  ...
  const webView=camera[3],webSy=Math.sin(webView.yaw),webCy=Math.cos(webView.yaw),webSt=Math.sin(webView.pitch),webCt=Math.cos(webView.pitch)
  const webCandidates=nodes.map((node,index)=>{const x=node.x*webCy+node.z*webSy,z=-node.x*webSy+node.z*webCy,y=node.y*webCt-z*webSt,depth=node.y*webSt+z*webCt;return{index,x,y,depth,importance:node.importance}}).filter(node=>node.depth>-.42)
  const webDistricts=[];while(webDistricts.length<3){const next=webCandidates.filter(node=>!webDistricts.some(center=>center.index===node.index)).sort((a,b)=>{const separation=node=>webDistricts.length?Math.min(...webDistricts.map(center=>Math.hypot(node.x-center.x,node.y-center.y))):1;return(b.importance*.28+separation(b)*.72)-(a.importance*.28+separation(a)*.72)})[0];if(!next)break;webDistricts.push(next)}
  nodes.webDistrictIndices=webDistricts.map(node=>node.index)
...
function paintWebWorld(ctx,nodes,model,weight,time) {
  ...
  const districts=centers.map(center=>({center,nodes:[]}));eligible.forEach(node=>{const district=districts.reduce((best,current)=>Math.hypot(node.x-current.center.x,node.y-current.center.y)<Math.hypot(node.x-best.center.x,node.y-best.center.y)?current:best,districts[0]),distance=Math.hypot(node.x-district.center.x,node.y-district.center.y),falloff=Math.exp(-Math.pow(distance/radius,2)*1.45),chance=.12+falloff*.88;if(node.index===district.center.index||hash(node.index,district.center.index+57)<chance)district.nodes.push({node,score:falloff*.72+node.importance*.2+node.seed*.08})})
  districts.forEach(district=>{district.nodes.sort((a,b)=>b.score-a.score);district.nodes= district.nodes.slice(0,14).map(item=>item.node);if(!district.nodes.some(node=>node.index===district.center.index))district.nodes.unshift(district.center)})

  districts.forEach((district,districtIndex)=>{const spreadX=Math.max(48,Math.min(175,Math.max(...district.nodes.map(node=>Math.abs(node.x-district.center.x)))+24)),spreadY=Math.max(24,Math.min(92,Math.max(...district.nodes.map(node=>Math.abs(node.y-district.center.y)))+15));ctx.save();ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.clip();ctx.strokeStyle=rgba(districtIndex===1?GOLD:SMOKE,.075*weight);ctx.lineWidth=.38;for(let offset=-spreadX;offset<=spreadX;offset+=18){ctx.beginPath();ctx.moveTo(district.center.x+offset-spreadY*.35,district.center.y-spreadY);ctx.lineTo(district.center.x+offset+spreadY*.35,district.center.y+spreadY);ctx.stroke()}for(let offset=-spreadY;offset<=spreadY;offset+=14){ctx.beginPath();ctx.moveTo(district.center.x-spreadX,district.center.y+offset);ctx.lineTo(district.center.x+spreadX,district.center.y+offset*.72);ctx.stroke()}ctx.restore();ctx.strokeStyle=rgba(districtIndex===1?GOLD:PEARL,.12*weight);ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.stroke()})
