...
function paintWebWorld(ctx,nodes,model,weight,time) {
  ...
  districts.forEach(district=>{const originalCenter={...district.center},horizon=viewportHeight*.57,project=node=>{const near=clamp01((node.z+1)/2),spread=.74+near*.82,floorY=lerp(horizon,viewportHeight*.88,near);return{...node,x:viewportWidth*.56+(node.x-originalCenter.x)*spread,y:floorY+(node.y-originalCenter.y)*(.42+near*.22)}};district.nodes=district.nodes.map(project);district.center=project(originalCenter)})
...
export function renderNasusOrb(ctx,model,{width,height,time,progress,regionalFocus,mode,reduced,pointer}) {
  ...
  if(webTravel>.001){const target=cameraTargetForAnchor(REGION_ANCHORS.web,time*WORLD_SPIN),yawDelta=Math.atan2(Math.sin(target.yaw-view.yaw),Math.cos(target.yaw-view.yaw)),orbitArc=Math.sin(webTravel*Math.PI);view.zoom=lerp(view.zoom,1.78,webTravel);view.yaw+=yawDelta*webTravel-orbitArc*.12;view.pitch=lerp(view.pitch,target.pitch-.24,webTravel);view.x=lerp(view.x,-.06,webTravel)-orbitArc*.03;view.y=lerp(view.y,.12,webTravel)}
