...
export function createOrbModel(count) {
  const nodes=Array.from({length:count},(_,i)=>{const [x,y,z]=fib(i,count),life=140+hash(i,8.1)*120;return{x,y,z,vx:(hash(i,2.1)-.5)*.04,vy:(hash(i,3.7)-.5)*.04,vz:(hash(i,6.4)-.5)*.04,seed:hash(i,4.7),phase:hash(i,9.3)*Math.PI*2,principal:i%9===0||i%13===0,alpha:1,age:hash(i,11.7)*life,life,generation:0,state:'alive'}})
...
export function renderNasusOrb(ctx,model,{width,height,time,progress,mode,reduced,pointer}) {
  ...
  const edges=smoothEdges(model,proximityEdges(nodes,profile.threshold+.05+(activity*.018),profile.neighbours+1+(mode==='thinking'?1:0),t,activity))
