...
function paintWhatsappWorld(ctx,nodes,hub,weight,time,model) {
  ...
  const participants=selectNodesAroundAnchor(nodes,REGION_ANCHORS.whatsapp,{innerAngle:.34,outerAngle:1.08,minAlpha:.12,limit:48}),allByIndex=new Map(nodes.map(node=>[node.index,node]))
  if(!model.whatsappTopology||time-model.whatsappTopology.createdAt>16){const scored=participants.map(node=>({node,density:participants.reduce((sum,other)=>sum+(other===node?0:Math.max(0,1-Math.hypot(node.x-other.x,node.y-other.y)/150)),0)})).sort((a,b)=>b.density-a.density),centers=[];scored.forEach(({node})=>{if(centers.length<4&&centers.every(center=>Math.hypot(center.x-node.x,center.y-node.y)>96))centers.push(node)});const groups=centers.map(center=>({center,members:[]}));participants.forEach(node=>{const nearest=groups.map(group=>({group,distance:Math.hypot(node.x-group.center.x,node.y-group.center.y)})).sort((a,b)=>a.distance-b.distance)[0];if(nearest&&nearest.group.members.length<15)nearest.group.members.push(node.index)});model.whatsappTopology={createdAt:time,clusters:groups.filter(group=>group.members.length>=8).map(group=>group.members)}}
  const activeClusters=model.whatsappTopology.clusters.map(cluster=>{const live=cluster.map(index=>allByIndex.get(index)).filter(Boolean),centerX=live.reduce((sum,node)=>sum+node.x,0)/Math.max(1,live.length),centerY=live.reduce((sum,node)=>sum+node.y,0)/Math.max(1,live.length);return live.map(node=>({...node,x:lerp(node.x,centerX,.48),y:lerp(node.y,centerY,.48)}))}).filter(cluster=>cluster.length>=8),secondary=activeClusters.map(cluster=>[...cluster].sort((a,b)=>b.importance-a.importance)[0]),densityByNode=new Map()
  activeClusters.forEach(cluster=>cluster.forEach(node=>densityByNode.set(node,Math.min(1,(cluster.length-1)/7))))
  ...
  activeClusters.forEach(cluster=>{cluster.forEach(node=>{[...cluster].filter(other=>other.index!==node.index).sort((a,b)=>Math.hypot(node.x-a.x,node.y-a.y)-Math.hypot(node.x-b.x,node.y-b.y)).slice(0,node.importance>.62?7:5).forEach(peer=>addDesired(node,peer))})});secondary.forEach((node,index)=>{if(secondary.length>1)addDesired(node,secondary[(index+1)%secondary.length])});if(secondary.length)addDesired(hub,secondary[Math.floor(time/5.5)%secondary.length],true)
  ...
  const populationZones=[[-178,-72,118,72],[132,-96,132,78],[-94,126,108,68],[188,92,124,74]]
  populationZones.forEach(([offsetX,offsetY,radiusX,radiusY],zone)=>{const driftX=Math.sin(time*.035+zone*1.7)*5,driftY=Math.cos(time*.028+zone*2.1)*3;for(let dot=0;dot<155;dot++){const angle=hash(dot+zone*181,104.3)*Math.PI*2,radial=Math.pow(hash(dot+zone*197,119.7),1.62),organic=.72+Math.sin(angle*(3+zone%2)+zone*1.4)*.18+Math.sin(angle*7+zone)*.08,x=hub.x+offsetX+driftX+Math.cos(angle)*radiusX*radial*organic,y=hub.y+offsetY+driftY+Math.sin(angle)*radiusY*radial*(.86+Math.cos(angle*4+zone)*.12),alpha=(.075+hash(dot,zone+132.4)*.21)*(1-radial*.34)*weight,size=.28+hash(dot+31,zone+9.7)*.54;ctx.fillStyle=rgba((dot+zone*11)%13===0?GOLD:PEARL,alpha);ctx.fillRect(x-size*.5,y-size*.5,size,size)}})
  ;[[0,1,.08],[0,2,.38],[1,3,.62],[2,3,.84]].forEach(([from,to,bend],bridge)=>{const a=populationZones[from],b=populationZones[to];for(let dot=0;dot<82;dot++){const progress=(dot+hash(dot,bridge+151.2)*.8)/82,envelope=Math.sin(progress*Math.PI),curve=Math.sin(progress*Math.PI)*((bend-.5)*95),x=hub.x+lerp(a[0],b[0],progress)+curve+Math.sin(time*.025+bridge)*3+(hash(dot,bridge+161.4)-.5)*34*envelope,y=hub.y+lerp(a[1],b[1],progress)-curve*.22+(hash(dot,bridge+173.8)-.5)*24*envelope,alpha=(.035+hash(dot,bridge+184.1)*.1)*envelope*weight,size=.25+hash(dot,bridge+194.6)*.42;ctx.fillStyle=rgba(dot%17===0?GOLD:PEARL,alpha);ctx.fillRect(x,y,size,size)}})
...
export function renderNasusOrb(ctx,model,{width,height,time,progress,regionalFocus,mode,reduced,pointer}) {
  ...
  globalNodes.sort((a,b)=>a.z-b.z).forEach(node=>{const depth=(node.z+1)/2,hub=node.importance>.64,weight=.58+node.importance*1.55,nodeScale=width<700?1.35:width<1100?1.2:1.8,nodeContrast=desktopReadability?1.56:1,r=Math.max(.2,.29+depth*.76)*weight*(hub?1.42:.78)*(width<700?.86:1)*nodeScale,opacity=(.09+depth*.68)*(.54+node.importance*.42)*(hub?1.32:.62)*globalDepthVisibility(node.z)*nodeContrast;ctx.fillStyle=rgba(hub&&node.index%5===region?GOLD:PEARL,opacity*node.alpha*globalOpacity);ctx.beginPath();ctx.arc(node.x,node.y,r+(activity*(hub?.7:.08)),0,Math.PI*2);ctx.fill()})
