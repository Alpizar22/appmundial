...
function paintWebTower(ctx,node,weight,time) {
  const depth=Math.max(.06,(node.z+1)/2),importance=.35+node.importance*.9,growth=smoothstep(weight),height=(20+node.seed*42+node.importance*82)*(.4+depth*.82)*growth,width=(5+node.importance*11)*(.5+depth*.76),lean=(node.seed-.5)*width*.7,topX=node.x+lean,topY=node.y-height,color=node.importance>.62?GOLD:PEARL,alpha=(.2+depth**1.55*.66)*weight*node.alpha,archetype=Math.floor(node.seed*3)
  ctx.strokeStyle=rgba(color,alpha);ctx.lineWidth=node.importance>.62?.85:.48;ctx.beginPath()
  ...
  ctx.moveTo(node.x-width,node.y);ctx.lineTo(node.x+width,node.y);ctx.stroke()
  const levels=3+Math.floor(height/19)+Math.floor(node.importance*2);for(let level=1;level<levels;level++){const progress=level/levels,y=lerp(node.y,topY,progress),x=lerp(node.x,topX,progress),setback=archetype===2&&progress>.48?.7:1,half=lerp(width,width*.45,progress)*setback;ctx.strokeStyle=rgba(level===levels-1&&node.importance>.62?GOLD:SMOKE,alpha*.55);ctx.beginPath();ctx.moveTo(x-half,y);ctx.lineTo(x+half,y);ctx.stroke()}
  const panelCount=node.seed>.12?(node.importance>.62||height>68?2:1):0;for(let panel=0;panel<panelCount;panel++){const progress=.28+frac(node.seed*3.7+panel*.41)*.48,panelY=lerp(node.y,topY,progress),panelX=lerp(node.x,topX,progress),panelWidth=width*(.72+frac(node.seed*7.1+panel)*.62),panelHeight=Math.max(3.5,height*(.065+frac(node.seed*5.3+panel)*.055));ctx.fillStyle=rgba('#171a20',alpha*.2);ctx.fillRect(panelX-panelWidth,panelY-panelHeight,panelWidth*2,panelHeight);ctx.strokeStyle=rgba(panel===1?GOLD:PEARL,alpha*.48);ctx.strokeRect(panelX-panelWidth,panelY-panelHeight,panelWidth*2,panelHeight)}
...
function paintWebWorld(ctx,nodes,model,weight,time) {
  ...
  const eligible=nodes.filter(node=>node.z>-.58&&node.alpha>.2&&node.x>viewportWidth*.08&&node.x<viewportWidth*.98&&node.y>viewportHeight*.015&&node.y<viewportHeight*.98)
  if(model.webDistrictIndices.length<3){const centerCandidates=eligible.filter(node=>node.z>-.32&&node.x>viewportWidth*.28&&node.x<viewportWidth*.9&&node.y>viewportHeight*.14&&node.y<viewportHeight*.82).map(node=>({...node,density:eligible.filter(other=>Math.hypot(node.x-other.x,node.y-other.y)<radius*.68).length})),selected=[];while(selected.length<3){const next=centerCandidates.filter(node=>node.density>=7&&!selected.some(center=>center.index===node.index)).sort((a,b)=>{const separation=node=>{if(!selected.length)return 1;const distance=Math.min(...selected.map(center=>Math.hypot(node.x-center.x,node.y-center.y)));return Math.exp(-Math.pow((distance-radius*.92)/(radius*.42),2))};const score=node=>Math.min(1,node.density/15)*.54+separation(node)*.34+node.importance*.12;return score(b)-score(a)})[0];if(!next)break;selected.push(next)}model.webDistrictIndices=selected.map(node=>node.index)}
  const centers=model.webDistrictIndices.map(index=>nodes.find(node=>node.index===index)).filter(Boolean)
  if(!centers.length)return
  const districts=centers.map(center=>({center,nodes:[],pool:[]}));eligible.forEach(node=>{const district=districts.reduce((best,current)=>Math.hypot(node.x-current.center.x,node.y-current.center.y)<Math.hypot(node.x-best.center.x,node.y-best.center.y)?current:best,districts[0]),distance=Math.hypot(node.x-district.center.x,node.y-district.center.y),falloff=Math.exp(-Math.pow(distance/radius,2)*1.45),chance=.12+falloff*.88;district.pool.push({node,distance});if(node.index===district.center.index||hash(node.index,district.center.index+57)<chance)district.nodes.push({node,score:falloff*.72+node.importance*.2+node.seed*.08})})
  districts.forEach(district=>{district.nodes.sort((a,b)=>b.score-a.score);const selected=district.nodes.slice(0,14).map(item=>item.node),selectedIds=new Set(selected.map(node=>node.index)),minimum=Math.min(10,district.pool.length);district.pool.sort((a,b)=>a.distance-b.distance).forEach(({node})=>{if(selected.length>=minimum||selectedIds.has(node.index))return;selected.push(node);selectedIds.add(node.index)});district.nodes=selected;if(!selectedIds.has(district.center.index))district.nodes.unshift(district.center)})

  districts.forEach((district,districtIndex)=>{const spreadX=Math.max(48,Math.min(155,Math.max(...district.nodes.map(node=>Math.abs(node.x-district.center.x)))+24)),spreadY=Math.max(24,Math.min(82,Math.max(...district.nodes.map(node=>Math.abs(node.y-district.center.y)))+15));ctx.save();ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.clip();ctx.strokeStyle=rgba(SMOKE,.13*weight);ctx.lineWidth=.42;for(let offset=-spreadX;offset<=spreadX;offset+=18){ctx.beginPath();ctx.moveTo(district.center.x+offset-spreadY*.35,district.center.y-spreadY);ctx.lineTo(district.center.x+offset+spreadY*.35,district.center.y+spreadY);ctx.stroke()}for(let offset=-spreadY;offset<=spreadY;offset+=14){ctx.beginPath();ctx.moveTo(district.center.x-spreadX,district.center.y+offset);ctx.lineTo(district.center.x+spreadX,district.center.y+offset*.72);ctx.stroke()}ctx.restore();ctx.strokeStyle=rgba(PEARL,.2*weight);ctx.lineWidth=.55;ctx.beginPath();ctx.ellipse(district.center.x,district.center.y,spreadX,spreadY,-.08+(districtIndex-1)*.06,0,Math.PI*2);ctx.stroke();ctx.fillStyle=rgba(districtIndex===1?GOLD:PEARL,.5*weight);ctx.beginPath();ctx.arc(district.center.x,district.center.y,1.5,0,Math.PI*2);ctx.fill()})

  districts.forEach(district=>{const streetKeys=new Set();district.nodes.forEach(node=>district.nodes.filter(peer=>peer.index!==node.index).sort((a,b)=>Math.hypot(a.x-node.x,a.y-node.y)-Math.hypot(b.x-node.x,b.y-node.y)).slice(0,2).forEach(peer=>{const key=node.index<peer.index?`${node.index}:${peer.index}`:`${peer.index}:${node.index}`;if(streetKeys.has(key))return;streetKeys.add(key);const depth=Math.max(.08,((node.z+peer.z)*.5+1)/2);ctx.strokeStyle=rgba(PEARL,.17*depth*weight);ctx.lineWidth=.58;ctx.beginPath();ctx.moveTo(node.x,node.y);ctx.lineTo(peer.x,peer.y);ctx.stroke()}))})
  districts.slice(0,-1).forEach((district,index)=>{const next=districts[index+1];ctx.strokeStyle=rgba(index===0?GOLD:PEARL,.055*weight);ctx.lineWidth=.48;ctx.setLineDash([3,7]);ctx.beginPath();ctx.moveTo(district.center.x,district.center.y);ctx.lineTo(next.center.x,next.center.y);ctx.stroke();ctx.setLineDash([])})

  const towers=districts.flatMap((district,districtIndex)=>district.nodes.map(node=>({...paintWebTower(ctx,node,weight,time),districtIndex}))).sort((a,b)=>a.z-b.z)
...
export function renderNasusOrb(ctx,model,{width,height,time,progress,mode,reduced,pointer}) {
  const regional=regionalStateAt(progress),region=regional.index,weights=regional.weights,profile=profileAt(regional),view=cameraAt(progress,regional)
  if(weights[2]>.001){
    const hub=model[model.communicationIndex],targetYaw=Math.atan2(-hub.x,hub.z)-time*WORLD_SPIN
    const yawDelta=Math.atan2(Math.sin(targetYaw-view.yaw),Math.cos(targetYaw-view.yaw))
    view.yaw+=yawDelta*weights[2]*.72
  ...
  paintRegionalLayer(ctx,nodes,model,weights,t)
}
