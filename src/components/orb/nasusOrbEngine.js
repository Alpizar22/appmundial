...
function paintWhatsappWorld(ctx,nodes,hub,weight,time,model) {
  ...
  if(!model.whatsappTopology||time-model.whatsappTopology.createdAt>16){const scored=participants.map(node=>({node,density:participants.reduce((sum,other)=>sum+(other===node?0:Math.max(0,1-Math.hypot(node.x-other.x,node.y-other.y)/150)),0)})).sort((a,b)=>b.density-a.density),centers=[];scored.forEach(({node})=>{if(centers.length<4&&centers.every(center=>Math.hypot(center.x-node.x,center.y-node.y)>112))centers.push(node)});const groups=centers.map(center=>({center,members:[]}));participants.forEach(node=>{const nearest=groups.map(group=>({group,distance:Math.hypot(node.x-group.center.x,node.y-group.center.y)})).sort((a,b)=>a.distance-b.distance)[0];if(nearest&&nearest.distance<190)nearest.group.members.push(node.index)});model.whatsappTopology={createdAt:time,clusters:groups.filter(group=>group.members.length>=4).map(group=>group.members)}}
  const activeClusters=model.whatsappTopology.clusters.map(cluster=>cluster.map(index=>allByIndex.get(index)).filter(Boolean)).filter(cluster=>cluster.length>=3),secondary=activeClusters.map(cluster=>[...cluster].sort((a,b)=>b.importance-a.importance)[0]),densityByNode=new Map()
  ...
  activeClusters.forEach(cluster=>{cluster.forEach(node=>{[...cluster].filter(other=>other!==node).sort((a,b)=>Math.hypot(node.x-a.x,node.y-a.y)-Math.hypot(node.x-b.x,node.y-b.y)).slice(0,node.importance>.62?3:2).forEach(peer=>addDesired(node,peer))});addDesired(hub,[...cluster].sort((a,b)=>b.importance-a.importance)[0],true)})
