...
function paintWhatsappWorld(ctx,nodes,hub,weight,time,model) {
  ...
  activeClusters.forEach(cluster=>{cluster.forEach(node=>{[...cluster].filter(other=>other.index!==node.index).sort((a,b)=>Math.hypot(node.x-a.x,node.y-a.y)-Math.hypot(node.x-b.x,node.y-b.y)).slice(0,node.importance>.62?7:5).forEach(peer=>addDesired(node,peer))});addDesired(hub,[...cluster].sort((a,b)=>b.importance-a.importance)[0],true)});secondary.forEach((node,index)=>{if(secondary.length>1)addDesired(node,secondary[(index+1)%secondary.length])})
  ...
  const routes=[...activeClusters.flat().slice(0,18),...secondary]
