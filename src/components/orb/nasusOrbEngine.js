...
export function createOrbModel(count) {
  ...
  const nodes=Array.from({length:count},(_,i)=>{const [x,y,z]=fib(i,count),transient=i%17===0,life=42+hash(i,8.1)*22,slot=transient?transientIndex++:0;return{x,y,z,vx:(hash(i,2.1)-.5)*.052,vy:(hash(i,3.7)-.5)*.052,vz:(hash(i,6.4)-.5)*.052,seed:hash(i,4.7),phase:hash(i,9.3)*Math.PI*2,principal:i%9===0||i%13===0,transient,alpha:1,age:transient?(slot/transientCount)*42:0,life,generation:0,state:'alive'}})
