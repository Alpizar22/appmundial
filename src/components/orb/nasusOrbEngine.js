...
import { REGION_ANCHORS, WORLD_HOTSPOTS, cameraTargetForAnchor, projectAnchor, selectNodesAroundAnchor } from './regions/regionAnchors'
...
const GOLD_ACTIVE = '#d6a64b'
const GOLD_MUTED = '#c4a882'
const TEXT_IVORY = '#e8e6e3'
const SMOKE_SECONDARY = '#8d9199'
...
// Coseno del angulo donde el hundimiento de voz llega a cero. z2 es la coordenada del nodo en
// el eje de camara, o sea el producto punto contra el punto del orbe que mira a la pantalla:
// el ancla fija que queremos. El falloff ya existia como `front`, pero con un radio de ~85 que
// abarcaba casi todo el hemisferio visible y por eso se leia parejo. .62 lo cierra a ~52, un
...
// Profundidad del hundimiento cuando el inputLevel del microfono llega a 1, a la misma escala
// que HOVER_DEPTH para que escuchar se sienta tan fisico como el cursor. Estaba en .065, que
...
function paintWhatsappRoute(ctx,hub,node,weight,width=.8) {
  ...
  gradient.addColorStop(0,rgba(GOLD_ACTIVE,.82*weight*depth))
  gradient.addColorStop(.42,rgba(GOLD_MUTED,.5*weight*depth))
  gradient.addColorStop(1,rgba(TEXT_IVORY,.2*weight*depth))
...
function paintWhatsappNetworkLine(ctx,a,b,hub,weight,width=.45,scale=1) {
  ...
  gradient.addColorStop(1,rgba(warmthB>.45?GOLD_ACTIVE:TEXT_IVORY,(.12+warmthB*.27)*weight*depth))
...
function paintSignalWave(ctx,hub,node,weight,time,index) {
  ...
  ctx.strokeStyle=rgba(index<2?GOLD_ACTIVE:TEXT_IVORY,(.2-index*.022)*weight);ctx.lineWidth=index<2?.72:.48;ctx.beginPath()
...
function createWhatsappPopulation() {
  ...
  for(let candidate=0;candidate<2500;candidate++){const offsetX=(hash(candidate,211.7)-.5)*620,offsetY=(hash(candidate,223.9)-.5)*390,density=zones.reduce((sum,[x,y,rx,ry])=>sum+Math.exp(-(Math.pow((offsetX-x)/rx,2)+Math.pow((offsetY-y)/ry,2))*1.42),0),acceptance=clamp01((density-.1)*1.02);if(hash(candidate,239.3)>acceptance)continue;points.push({index:10000+points.length,offsetX,offsetY,density:clamp01(density*.58),importance:.18+Math.pow(hash(candidate,251.1),4)*.72,size:.8+hash(candidate,263.5),alpha:.17+hash(candidate,277.9)*.28})}
  return points
}

function createWhatsappPopulationTexture(points) {
  if(typeof document==='undefined')return null
  const width=640,height=400,canvas=document.createElement('canvas'),texture=canvas.getContext('2d');canvas.width=width;canvas.height=height
  points.forEach((point,index)=>{const collective=.72+point.density*.78;texture.fillStyle=rgba(index%29===0?GOLD_ACTIVE:TEXT_IVORY,point.alpha*collective);texture.fillRect(width*.5+point.offsetX-point.size*.5,height*.5+point.offsetY-point.size*.5,point.size,point.size)})
  return canvas
}
...
function paintWebWorld(ctx,nodes,model,weight,time,viewportWidth,viewportHeight) {
  ...
  districts.forEach(district=>{const bounds=district.nodes.reduce((box,node)=>{const depth=Math.max(.03,(node.z+1)/2),towerHeight=(20+node.seed*42+node.importance*82)*(.4+depth*.82)*1.08;box.left=Math.min(box.left,node.x);box.right=Math.max(box.right,node.x);box.top=Math.min(box.top,node.y-towerHeight);box.bottom=Math.max(box.bottom,node.y);return box},{left:Infinity,right:-Infinity,top:Infinity,bottom:-Infinity}),offsetX=viewportWidth*.5-(bounds.left+bounds.right)*.5,offsetY=viewportHeight*.62-(bounds.top+bounds.bottom)*.5;district.nodes=district.nodes.map(node=>({...node,x:node.x+offsetX,y:node.y+offsetY}));district.center={...district.center,x:district.center.x+offsetX,y:district.center.y+offsetY}})

  paintWebSky(ctx,viewportWidth,viewportHeight,weight,time)
  const vanishingX=viewportWidth*.5,horizon=viewportHeight*.55,groundBottom=viewportHeight*1.015;ctx.save();ctx.beginPath();ctx.moveTo(vanishingX-viewportWidth*.11,horizon);ctx.lineTo(vanishingX+viewportWidth*.11,horizon);ctx.lineTo(viewportWidth*1.08,groundBottom);ctx.lineTo(-viewportWidth*.08,groundBottom);ctx.closePath();ctx.clip();ctx.strokeStyle=rgba(SMOKE_SECONDARY,.2*weight);ctx.lineWidth=.5;for(let lane=-8;lane<=8;lane++){ctx.beginPath();ctx.moveTo(vanishingX+lane*viewportWidth*.012,horizon);ctx.lineTo(vanishingX+lane*viewportWidth*.082,groundBottom);ctx.stroke()}for(let row=1;row<=11;row++){const t=row/11,y=lerp(horizon,groundBottom,t**1.72),half=lerp(viewportWidth*.11,viewportWidth*.58,t);ctx.beginPath();ctx.moveTo(vanishingX-half,y);ctx.lineTo(vanishingX+half,y);ctx.stroke()}ctx.restore();ctx.strokeStyle=rgba(TEXT_IVORY,.3*weight);ctx.lineWidth=.65;ctx.beginPath();ctx.moveTo(vanishingX-viewportWidth*.11,horizon);ctx.lineTo(-viewportWidth*.08,groundBottom);ctx.moveTo(vanishingX+viewportWidth*.11,horizon);ctx.lineTo(viewportWidth*1.08,groundBottom);ctx.stroke();districts.forEach(district=>{ctx.fillStyle=rgba(GOLD_ACTIVE,.72*weight);ctx.beginPath();ctx.arc(district.center.x,district.center.y,2,0,Math.PI*2);ctx.fill()})

  const streetEdges=[];districts.forEach((district,districtIndex)=>{const streetKeys=new Set();district.nodes.forEach(node=>district.nodes.filter(peer=>peer.index!==node.index).sort((a,b)=>Math.hypot(a.x-node.x,a.y-node.y)-Math.hypot(b.x-node.x,b.y-node.y)).slice(0,node.importance>.62?3:2).forEach(peer=>{const key=node.index<peer.index?`${node.index}:${peer.index}`:`${peer.index}:${node.index}`;if(streetKeys.has(key))return;streetKeys.add(key);const depth=Math.max(.03,((node.z+peer.z)*.5+1)/2),fog=.38+smoothstep(depth)*.62;ctx.strokeStyle=rgba(TEXT_IVORY,.29*fog*weight);ctx.lineWidth=.84;ctx.beginPath();ctx.moveTo(node.x,node.y);ctx.lineTo(peer.x,peer.y);ctx.stroke();streetEdges.push({a:node,b:peer,districtIndex,depth:fog})}))})
  const intercityEdges=[];districts.slice(0,-1).forEach((district,index)=>{const next=districts[index+1];ctx.strokeStyle=rgba(index===0?GOLD_ACTIVE:TEXT_IVORY,.038*weight);ctx.lineWidth=.34;ctx.setLineDash([2,9]);ctx.beginPath();ctx.moveTo(district.center.x,district.center.y);ctx.lineTo(next.center.x,next.center.y);ctx.stroke();ctx.setLineDash([]);intercityEdges.push({a:district.center,b:next.center})})
  ...
  towers.forEach(tower=>{const peers=towers.filter(other=>other.districtIndex===tower.districtIndex&&other.index!==tower.index).sort((a,b)=>Math.hypot(a.x-tower.x,a.y-tower.y)-Math.hypot(b.x-tower.x,b.y-tower.y)).slice(0,tower.importance>.62?2:1);peers.forEach((peer,peerIndex)=>{const depth=Math.max(.06,(tower.depth+peer.depth)/2),active=tower.importance>.62||peer.importance>.62;ctx.strokeStyle=rgba(active&&peerIndex===0?GOLD_ACTIVE:TEXT_IVORY,(active?.2:.105)*depth*weight);ctx.lineWidth=active?.7:.42;ctx.beginPath();ctx.moveTo(tower.topX,tower.topY);ctx.lineTo(peer.topX,peer.topY);ctx.stroke()})})
  towers.forEach((tower,index)=>{const footprint=tower.width*1.35;ctx.strokeStyle=rgba(index%7===0?GOLD_ACTIVE:SMOKE_SECONDARY,(.12+tower.depth*.14)*weight);ctx.lineWidth=.48;ctx.beginPath();ctx.moveTo(tower.x-footprint,tower.y);ctx.lineTo(tower.x,tower.y+footprint*.35);ctx.lineTo(tower.x+footprint,tower.y);ctx.lineTo(tower.x,tower.y-footprint*.35);ctx.closePath();ctx.stroke()})
  for(let packet=0;packet<22&&streetEdges.length;packet++){const edge=streetEdges[(packet*7)%streetEdges.length],phase=frac(time*(.07+(packet%4)*.009)+packet*.119),envelope=Math.sin(phase*Math.PI),x=lerp(edge.a.x,edge.b.x,phase),y=lerp(edge.a.y,edge.b.y,phase);ctx.fillStyle=rgba(packet%5===0?GOLD_ACTIVE:TEXT_IVORY,envelope*edge.depth*.95*weight);ctx.beginPath();ctx.arc(x,y,1+edge.depth*.78,0,Math.PI*2);ctx.fill()}
  intercityEdges.forEach((edge,index)=>{const phase=frac(time*.028+index*.43),envelope=Math.sin(phase*Math.PI),x=lerp(edge.a.x,edge.b.x,phase),y=lerp(edge.a.y,edge.b.y,phase);ctx.fillStyle=rgba(index===0?GOLD_ACTIVE:TEXT_IVORY,envelope*.24*weight);ctx.beginPath();ctx.arc(x,y,.75,0,Math.PI*2);ctx.fill()})
}
...
function paintCasesHotspot(ctx,point,visibility,time) {
  if(visibility<.015)return
  const pulse=.5+.5*Math.sin(time*.72),radius=15+pulse*3
  ctx.save();ctx.translate(point.x,point.y)
  const glow=ctx.createRadialGradient(0,0,1,0,0,28);glow.addColorStop(0,rgba(GOLD_ACTIVE,.34*visibility));glow.addColorStop(.34,rgba(GOLD_ACTIVE,.1*visibility));glow.addColorStop(1,rgba(GOLD_ACTIVE,0));ctx.fillStyle=glow;ctx.fillRect(-28,-28,56,56)
  ctx.strokeStyle=rgba(GOLD_ACTIVE,(.28+pulse*.1)*visibility);ctx.lineWidth=.7;ctx.beginPath();ctx.arc(0,0,radius,0,Math.PI*2);ctx.stroke()
  ctx.strokeStyle=rgba(TEXT_IVORY,.22*visibility);ctx.beginPath();ctx.arc(0,0,9,0,Math.PI*2);ctx.stroke()
...
export function renderNasusOrb(ctx,model,{width,height,time,progress,regionalFocus,mode,voiceSignals,reduced,pointer}) {
  ...
  const automationWhatsappProgress=clamp01((progress-1.12)/.76),automationWhatsappTransit=progress>1.12&&progress<1.88?Math.sin(automationWhatsappProgress*Math.PI):0
  if(automationWhatsappTransit>0){const easedArc=smoothstep(automationWhatsappTransit);view.yaw-=easedArc*.18;view.x-=Math.sin(automationWhatsappProgress*Math.PI)*.042;view.y+=Math.sin(automationWhatsappProgress*Math.PI*2)*.011;view.zoom-=easedArc*.105}
  const whatsappWebProgress=clamp01((progress-2.12)/.76),whatsappWebTransit=progress>2.12&&progress<2.88?Math.sin(whatsappWebProgress*Math.PI):0
  ...
  const webFocus=projectAnchor(REGION_ANCHORS.web,{yaw:view.yaw,pitch:view.pitch,worldRotation:t*WORLD_SPIN,base,cx,cy})
  ...
  // conservar la proporcion halo/orbe. Desktop y tablet (>=900px) quedan intactos.
