...
export default function Orb({ region, mode, onActivate }) {
  ...
  useEffect(()=>{
    ...
    const resize=()=>{const box=canvas.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,2);width=box.width;height=box.height;canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);const count=width<700?210:width<1100?300:360;if(model.length!==count)model=createOrbModel(count)}
