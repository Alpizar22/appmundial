...
export default function Orb({ region, mode, onActivate, onOpenCases }) {
  ...
  useEffect(()=>{
    ...
    const trackScroll=()=>{const vh=Math.max(innerHeight,1),raw=Math.max(0,(scrollY-vh*.72)/(vh*1.34)),stretched=raw<=1.25?raw/1.25:raw<=2.5?1+(raw-1.25)/1.25:raw<=3.75?2+(raw-2.5)/1.25:raw<=5?3+(raw-3.75)/1.25:raw-1;scrollTarget=Math.max(0,Math.min(4,stretched));journeyTarget=Math.max(0,Math.min(1,(scrollY-vh*.58)/(vh*.28)))}
