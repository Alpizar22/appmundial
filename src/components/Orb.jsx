...
export default function Orb({ region, mode, voiceSignals, onActivate, onOpenCases }) {
  ...
  useEffect(()=>{
    ...
    const resize=()=>{const box=canvas.getBoundingClientRect();width=box.width;height=box.height;const dpr=Math.min(devicePixelRatio||1,width<700?1:width<1100?1.6:2);canvas.width=Math.round(width*dpr);canvas.height=Math.round(height*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);const count=width<700?122:width<1100?180:360;if(model.length!==count)model=createOrbModel(count)}
